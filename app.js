...
(() => {
  ...
  function linkifyDescription(value) {
    ...
    raw.replace(urlPattern, (url, _match, offset) => {
      ...
      result += `<a href="${escapeAttr(cleanUrl)}" target="_blank" rel="noopener noreferrer" class="media-description-link">${escapeHtml(cleanUrl)}</a>`;
  ...
  function renderMediaGrid(id, items = [], cardClass) {
    ...
    grid.innerHTML = items.map((item) => {
      ...
      return `<article class="${cardClass}">${media}${item.description ? `<p>${linkifyDescription(item.description)}</p>` : ""}</article>`;
    }).join("");
