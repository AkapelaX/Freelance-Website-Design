...
alter table public.website_projects drop constraint if exists website_projects_domain_status_check;
alter table public.website_projects add constraint website_projects_domain_status_check
check (domain_status in ('not_connected','verifying','connected','failed','removing'));

alter table public.website_projects drop constraint if exists website_projects_ssl_status_check;
alter table public.website_projects add constraint website_projects_ssl_status_check
...
on public.website_projects (lower(custom_domain)) where custom_domain is not null;

create unique index if not exists website_projects_slug_unique
on public.website_projects (lower(slug)) where slug is not null;
